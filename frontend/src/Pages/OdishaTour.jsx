...
import { MapPin, Calendar, Users, Star, ArrowLeft, Phone, Mail, Umbrella, Search, X, Grid3x3, List, CheckCircle } from "lucide-react";
import { odishaDistrictsData } from "../data/odishaDistrictsData";

export default function OdishaTour() {
  ...
  const [bookingDistrict, setBookingDistrict] = useState(null);
  ...
  const filteredDistricts = useMemo(() => {
    if (!searchQuery.trim()) {
      return odishaDistrictsData.slice(0, 10); // Show only first 10 by default
    }
    
    const query = searchQuery.toLowerCase();
    return odishaDistrictsData.filter(district => 
      district.district.toLowerCase().includes(query) ||
      district.description.toLowerCase().includes(query) ||
      district.touristPlaces.some(place => place.toLowerCase().includes(query))
    );
  }, [searchQuery]);

  const handleBooking = (district) => {
    setBookingDistrict(district);
  ...
  const handleBookingSubmit = (e) => {
    e.preventDefault();
    console.log("Booking submitted:", { ...bookingData, district: bookingDistrict.district });
    alert(`Thank you! Your booking request for ${bookingDistrict.district} has been submitted. We'll contact you soon!`);
  ...
  return (
    <>
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-16">
          ...
          <div className="max-w-2xl mx-auto mb-8 md:mb-12">
            <div className="relative mb-4">
              ...
              <input
                type="text"
                placeholder="Search by district or tourist place..."
                value={searchQuery}
            ...
            {searchQuery && (
              <p className="text-center text-sm text-gray-600 mt-3">
                Found {filteredDistricts.length} result{filteredDistricts.length !== 1 ? 's' : ''}
              </p>
            )}
          </div>
          ...
          {filteredDistricts.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No districts or places found matching "{searchQuery}"</p>
              <button 
          ...
          {/* Show All Button */}
          {!searchQuery && filteredDistricts.length === 10 && (
            <div className="text-center mt-8 md:mt-12">
              <p className="text-gray-600 mb-4">
                Showing 10 of {odishaDistrictsData.length} districts. Use the search bar to find specific destinations.
              </p>
            </div>
          )}
        </section>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="bg-white px-4 py-2 rounded-full flex items-center gap-2 shadow-lg">
                      ...
                      <span className="font-bold text-lg">{selectedDistrict.rating}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Booking Modal */}
      <AnimatePresence>
        {bookingModalOpen && bookingDistrict && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
